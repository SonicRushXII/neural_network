...
public class Engine implements Serializable {
    ...
    private final LossFunction lossFunction = LossFunction.CROSSENTROPY;
    ...
    public BatchResult runForwards(Matrix input)
    {
        ...
        for(var t: transforms)
        {
            if(t == Transform.DENSE)
            {
                ...
                output = weight.multiplyMatrix(output).modify((row,col,value) -> value + bias.get(row));
    ...
    public void adjust(BatchResult batchResult, double learningRate){
        ...
        for(int i=0; i<weights.size(); ++i){
            ...
            var weightAdjust = error.multiplyMatrix(input.transpose());
            var biasAdjust = error.averageColumn();

            weight.modify((index,value)->value - (learningRate/input.getCols())*weightAdjust.get(index));
    ...
    public void runBackwards(BatchResult batchResult, Matrix expected)
    {
        ...
        if(lossFunction != LossFunction.CROSSENTROPY || transforms.getLast() != Transform.SOFTMAX) {
            throw new UnsupportedOperationException("Loss function must be CrossEntropy and Last Transform must be Softmax");
        }
            ...
            switch(transform){
                case DENSE:
                    ...
                    if(weightIt.hasNext() || storeInputError) {
                        error = weight.transpose().multiplyMatrix(error);
                    }
                    break;
                case RELU:
